    ...
    if user_waiting_for_image.get(user_id) == "waiting_prompt":
        ...
        try:
            ...
            if response.status_code == 200:
                await update.message.reply_photo(photo=response.content, caption="✅ عکس ادیت شد!")else:
                error = response.json()
                logger.error(f"Stability error: {error}")
                await update.message.reply_text("❌ خطا در ادیت عکس. دوباره امتحان کن.")
